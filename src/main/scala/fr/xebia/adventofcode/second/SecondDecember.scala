...
/**
  ...
  */
case class SecondDecember(values: Array[Array[Int]]) {

  def computeFirstPart(): Int = {
    values.map(list => list.max - list.min).sum
  }
  ...
  def computeSecondPart(): Int = {
    values.flatMap(list => {
      list.flatMap(i => {
        list
          .filter(_ != i)
          .find(SecondDecember.isDivisible(i))
          .map(i / _)
      })
    }).sum
  }
...
object SecondDecember {
  def apply(input: Array[String]): SecondDecember = {
    Try(input.map(_.split("\\W+").map(_.toInt))) match {
      case Success(l) => SecondDecember(l)
      case Failure(_) => throw new IllegalArgumentException(s"The input is invalid, it should only contains numbers")
    }
  }

  def isDivisible(i1: Int)(i2: Int): Boolean = {
    i1 % i2 == 0
  }
  ...
  def computeInput(): Unit = {
    ...
    val secondDecember = SecondDecember(input)
    val firstResult = secondDecember.computeFirstPart()
    val secondResult = secondDecember.computeSecondPart()

    println(s"Result for the first part of the 2nd December : $firstResult")
    println(s"Result for the second part of the 2nd December : $secondResult")
  }
}
